...
class ColorStack(Task):
    ...
    def _create_scene(self) -> None:
        ...
        self.sim.create_box(
            body_name="block_2",
            half_extents=np.array([1.2,1.2,1.2]) * self.object_size/2,
            mass=1.0,
            position=np.array([0.3, 0.5,1.2*self.object_size / 2]),
            rgba_color=np.array([0.1, 0.1, 0.9, 1.0]),
        )
        self.sim.create_box(
            body_name="block_3",
        ...
        self.sim.create_box(
            ...
            half_extents=np.array([1.2,1.2,1.2])* self.object_size / 2,
            mass=1.0,
            position=np.array([0.6, 0.0, 1.2*self.object_size / 2]),
            rgba_color=np.array([0.1, 0.9, 0.1, 1.0]),
        )
        ...
        self.sim.create_box(
            ...
            half_extents=np.ones(3) * self.object_size / 2,
            mass=1.0,
            position=np.array([0.6, -0.3, self.object_size / 2]),
        ...
        self.sim.create_box(
            ...
            half_extents=np.array([1.2,1.2,1.2])* self.object_size/2,
            mass=1.0,
            position=np.array([0.6, -0.5, 1.2*self.object_size / 2]),
            rgba_color=np.array([0.9, 0.1, 0.1, 1.0]),
        )
    ...
    def get_obs(self) -> np.ndarray:
        ...
        obs = {
            ...
            "block_6": np.array(self.sim.get_base_position("block_6")),
            ...
            "block_6_rotation": np.array(self.sim.get_base_rotation("block_6","euler"))
        }
        return obs
    ...
    def reset(self) -> None:
        ...
        block_1_position, block_2_position, block_3_position, block_4_position, block_5_position,block_6_position = self._sample_objects()
        ...
        self.sim.set_base_pose("block_6", block_6_position, np.array([0.,0.,0.,1.]))
    ...
    def _sample_objects(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        block_2_position = np.array([0.3, 0.5, 1.2*self.object_size / 2])
        block_3_position = np.array([0.6, 0.4, self.object_size / 2])
        block_4_position = np.array([0.6, 0.0, 1.2*self.object_size / 2])
        block_5_position = np.array([0.6, -0.3, self.object_size / 2])
        block_6_position = np.array([0.6, -0.5, 1.2*self.object_size / 2])
        return block_1_position, block_2_position, block_3_position, block_4_position, block_5_position, block_6_position
    ...
    def _get_object_orietation(self):
        ...
        block6_rotation=np.array(self.sim.get_base_rotation("block_6","quaternion"))
        return block1_rotation,block2_rotation, block3_rotation, block4_rotation, block5_rotation, block6_rotation
